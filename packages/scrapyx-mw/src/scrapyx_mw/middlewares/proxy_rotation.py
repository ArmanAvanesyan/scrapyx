...
class ProxyRotationMiddleware:
    ...
    def __init__(self, settings):
        ...
        self.rotation_strategy = settings.get(
            "SCRAPYX_PROXY_ROTATION_STRATEGY", "round_robin"
        )
        self.health_check_enabled = settings.getbool("SCRAPYX_PROXY_HEALTH_CHECK", True)
        self.health_check_interval = settings.getint(
            "SCRAPYX_PROXY_HEALTH_CHECK_INTERVAL", 300
        )
        self.max_failures = settings.getint("SCRAPYX_PROXY_MAX_FAILURES", 3)
        self.session_persistence = settings.getbool(
    ...
    def _get_weighted_proxy(self, proxies: List[str]) -> str:
        ...
        weights = []
        for proxy in proxies:
            stats = self.proxy_stats.get(
                proxy, {"success_rate": 1.0, "avg_response_time": 1.0}
            )
        ...
        if proxy:
            ...
            if success:
                ...
                spider.logger.warning(
                    f"Proxy {proxy} failed for {request.url} (status: {response.status})"
                )
    ...
    def process_exception(
        ...
        if proxy:
            ...
            spider.logger.warning(
                f"Proxy {proxy} failed for {request.url}: {exception}"
            )
