...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            resp = {"status":"healthy","timestamp":datetime.now().isoformat(),"solutions_count": self._count()}
    ...
    def _count(self) -> int:
        try:
            ...
            n = c.fetchone()[0]; conn.close(); return int(n)
        except Exception:
            return 0
...
class WebhookService:
    def __init__(self, host="127.0.0.1", port=6801):
        ...
        t = threading.Thread(target=self._cleanup, daemon=True); t.start()

    def start(self):
        try:
            self.server = HTTPServer((self.host, self.port), WebhookHandler)
            logger.info("Webhook service on %s:%s (DB: %s)", self.host, self.port, DB_PATH)
            self.server.serve_forever()
        except Exception as e:
            logger.error("Start failed: %s", e); raise

    def stop(self):
        if self.server:
            self.server.shutdown(); logger.info("Webhook service stopped")
    ...
    def _cleanup(self):
        while True:
            try:
                ...
                c.execute("DELETE FROM captcha_solutions WHERE received_at < ?", (cutoff,))
                deleted = c.rowcount; conn.commit(); conn.close()
                if deleted > 0: logger.info("Cleaned up %s old solutions", deleted)
            except Exception as e:
...
def get_solution(captcha_id: str) -> str | None:
    try:
        ...
        logger.error("Get solution error: %s", e); return None
